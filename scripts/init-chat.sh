...
set -a && source .env.chat.private && set +a
root_dir="$(pwd)"
...
# dir setup
log "dp::hermes::hermes_net_cloud::(busy)::creating folder structure."
cd ./chat/server
mkdir data
mkdir data/bridge
...
# prepare config
echo "dp::hermes::hermes_net_cloud::(busy)::preparing Maevis (Chat: Matrix) configuration files."
...
# log "dp::hermes::hermes_net_cloud::(busy)::preparing Hermes Matrix Home Server configuration files."
...
log "dp::hermes::hermes_net_cloud::(busy)::preparing Hermes Matrix Client configuration files."
